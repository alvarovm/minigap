...
#                     print("valid_ens[:3] = {}".format( valid_ens_j[:3].flatten()) )
    ...
    def weight(self):
        ...
        """
        self.gpr_model = gpflow.models.GPR( data=(self.train.sps_full, self.train.ens), kernel=self.kernel, noise_variance = self.gpr_model.likelihood.variance)
        
#         print_summary(self.gpr_model)
        
        if self.settings.prediction_calculation in ("direct", "cholesky"):
            KNN = self.gpr_model.kernel(self.train.sps_full)
            KNN_diag = tf.linalg.diag_part(KNN)
            variance_diag = tf.fill(tf.shape(KNN_diag), self.gpr_model.likelihood.variance)
            ...
            if s.prediction_calculation == "direct":
                ...
                self.trained_weights = tf.matmul(KNN_inv, train_ens)
            else:
                ...
                if self.settings.prediction_calculation == "predict_f":
                   ...
                   ### vama THIS IS A MESS 
                    ...
                    predict_ens = tf.reshape( predict_energies_from_weights_tf(self.trained_weights,
                                                                               self.train.sps_full, testdata.sps_full, degree=2), [-1,1])
                if self.settings.use_forces and not testdata is None:
